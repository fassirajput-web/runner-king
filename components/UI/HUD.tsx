...
import { Heart, Zap, Trophy, MapPin, Diamond, Rocket, ArrowUpCircle, Shield, Activity, PlusCircle, Play, Pause, RotateCcw, LogOut } from 'lucide-react';
...
const PauseScreen: React.FC = () => {
    const { togglePause, restartGame, exitToMenu } = useStore();

    return (
        <div className="absolute inset-0 bg-black/70 z-[100] text-white pointer-events-auto backdrop-blur-md flex items-center justify-center">
            <div className="flex flex-col items-center max-w-sm w-full p-8 rounded-3xl border border-white/10 bg-gray-900/50 shadow-2xl">
                <h2 className="text-4xl font-black text-cyan-400 mb-8 font-cyber tracking-widest">PAUSED</h2>
                
                <div className="flex flex-col gap-4 w-full">
                    <button 
                        onClick={togglePause}
                        className="flex items-center justify-center px-6 py-4 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl transition-all group"
                    >
                        <Play className="mr-3 w-5 h-5 group-hover:scale-110 transition-transform" fill="currentColor" />
                        <span className="font-bold text-lg">RESUME</span>
                    </button>
                    
                    <button 
                        onClick={restartGame}
                        className="flex items-center justify-center px-6 py-4 bg-cyan-600/20 hover:bg-cyan-600/40 border border-cyan-500/30 rounded-xl transition-all group"
                    >
                        <RotateCcw className="mr-3 w-5 h-5 group-hover:rotate-[-45deg] transition-transform" />
                        <span className="font-bold text-lg">RESTART</span>
                    </button>
                    
                    <button 
                        onClick={exitToMenu}
                        className="flex items-center justify-center px-6 py-4 bg-pink-600/20 hover:bg-pink-600/40 border border-pink-500/30 rounded-xl transition-all group"
                    >
                        <LogOut className="mr-3 w-5 h-5 group-hover:translate-x-1 transition-transform" />
                        <span className="font-bold text-lg">EXIT MISSION</span>
                    </button>
                </div>
            </div>
        </div>
...
export const HUD: React.FC = () => {
  const { score, lives, maxLives, collectedLetters, status, level, restartGame, startGame, gemsCollected, distance, isImmortalityActive, speed, togglePause } = useStore();
              ...
              <div className="relative w-full max-w-md rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,255,255,0.2)] border border-white/10 animate-in zoom-in-95 duration-500">
                <div className="relative w-full bg-gray-900">
                     ...
                     <div className="absolute inset-0 flex flex-col justify-end items-center p-6 pb-8 text-center z-10">
                        <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 mb-4 font-cyber tracking-widest drop-shadow-[0_0_15px_rgba(0,255,255,0.5)]">
                          RUNNER KING
                        </h1>
                        <button 
                        ...
                        >
                            ...
                            <span className="relative z-10 tracking-widest flex items-center justify-center">
                                PLAY <Play className="ml-2 w-5 h-5 fill-white" />
                            </span>
                        </button>
              ...
              <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                ...
                <div className="grid grid-cols-1 gap-3 md:gap-4 text-center mb-8 w-full max-w-md">
                    <div className="bg-gray-900/80 p-3 md:p-4 rounded-lg border border-gray-700 flex items-center justify-between">
                        ...
                        <div className="text-xl md:text-2xl font-bold font-mono">{level} / 100</div>
                    </div>
                    <div className="bg-gray-900/80 p-3 md:p-4 rounded-lg border border-gray-700 flex items-center justify-between">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-purple-900/90 to-black/95 z-[100] text-white pointer-events-auto backdrop-blur-md overflow-y-auto">
            <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                ...
                <p className="text-cyan-300 text-sm md:text-2xl font-mono mb-8 tracking-widest text-center">
                    LEVEL 100 REACHED: COSMOS CONQUERED
                </p>
    ...
    <div className={containerClass}>
        <div className="flex justify-between items-start w-full">
            ...
            <div className="flex items-center space-x-3 md:space-x-6">
                ...
                <button 
                    onClick={togglePause}
                    className="pointer-events-auto p-2 bg-white/5 hover:bg-white/20 border border-white/10 rounded-full transition-colors group"
                >
        ...
        <div className="absolute top-5 left-1/2 transform -translate-x-1/2 text-sm md:text-lg text-purple-300 font-bold tracking-wider font-mono bg-black/50 px-3 py-1 rounded-full border border-purple-500/30 backdrop-blur-sm z-50">
            LEVEL {level} <span className="text-gray-500 text-xs md:text-sm">/ 100</span>
        </div>
        {isImmortalityActive && (
